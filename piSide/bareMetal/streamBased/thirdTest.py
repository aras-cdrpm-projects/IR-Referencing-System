...
videoCmd = "raspividyuv -w "+str(w)+" -h "+str(h)+" --output - --timeout 0 --framerate "+str(fps)+" --luma --nopreview" #"-ex off -dg 2.4"
...
def recv(port=50000, addr="10.42.0.1", buf_size=1024):
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
while i<1000:
    ...
    #print(qmat)
    send(struct.pack('dddd',qmat[0], qmat[1], qmat[2], qmat[3]))
    #time.sleep(0.01)
    i=i+1
