...
class markerExteractor(object):
    def __init__(self):
        ...
        self.blubParams.minConvexity = 0.5
...
objNames = ['LR', 'LL_L', 'LL_H', 'UL', 'UR']
obj_points = np.zeros((5, 3), dtype=np.float)
for obj in objNames:
...
while len(points)!=5:
    if verboseFault==0:
        print("Detected "+str(len(points))+" instead of 5 markers. Wrote the most recent image to script directory")
        verboseFault=1
...
rec=np.hstack((np.array(heapq.nsmallest(2, range(len(centers[:, 1])), centers[:, 1].take)),
                  np.array(heapq.nlargest(2, range(len(centers[:, 1])), centers[:, 1].take))))
originx = 0
originy = 0
for index in rec:
    originx = originx+centers[index][0]
    originy = originy+centers[index][1]
originx = originx/len(rec)
originy = originy/len(rec)
...
LED=np.array([int(centers[(center[:, 0]>0) & (center[:, 1]>0)][0][0]),int(centers[(center[:, 0]>0) & (center[:, 1]>0)][0][1]),
             ...
             int(centers[(center[:, 0]>0) & (center[:, 1]<0)][0][0]),int(centers[(center[:, 0]>0) & (center[:, 1]<0)][0][1])]).reshape(5,2)
frame = img.copy()
frame = frameLabel(frame, LED)
ret, rvec, T = cv2.solvePnP(obj_points, LED.astype('float').reshape(5, 1, 2), marker_udistorder.K, marker_udistorder.D, flags=cv2.SOLVEPNP_ITERATIVE)
...
while cap.isOpened() and not rospy.is_shutdown():
    ...
    alpha = np.matmul(np.linalg.inv(R), T.reshape(3,-1))[2]/(np.matmul(RKin, np.vstack([uLED.transpose(), np.ones([1, 5])])))[2]
    GP = alpha*np.matmul(RKin, np.vstack([uLED.transpose(), np.ones([1, 5])]))-np.matmul(np.linalg.inv(R), T.reshape(3,-1))
    GP = GP.transpose()[:, 0:2]
    ...
    origin = (originx, originy)
    center = centers-origin
    GPstar=np.array([centers[(center[:, 0]>0) & (center[:, 1]>0)][0][0],centers[(center[:, 0]>0) & (center[:, 1]>0)][0][1],
                     centers[centers[:, 1]==np.max(centers[(center[:, 0]<0) & (center[:, 1]>0)][:, 1])][0][0],centers[centers[:, 1]==np.max(centers[(center[:, 0]<0) & (center[:, 1]>0)][:, 1])][0][1],
                     centers[centers[:, 1]==np.min(centers[(center[:, 0]<0) & (center[:, 1]>0)][:, 1])][0][0],centers[centers[:, 1]==np.min(centers[(center[:, 0]<0) & (center[:, 1]>0)][:, 1])][0][1],
                     centers[(center[:, 0]<0) & (center[:, 1]<0)][0][0],centers[(center[:, 0]<0) & (center[:, 1]<0)][0][1],
                     centers[(center[:, 0]>0) & (center[:, 1]<0)][0][0],centers[(center[:, 0]>0) & (center[:, 1]<0)][0][1]]).reshape(5,2)
    GPstar = GPstar.reshape(-1, 2)
    LED = np.array([points[np.where(GP==GPstar[0])],
            ...
            points[np.where(GP==GPstar[4])]]).reshape(5,2)
    frame = img.copy()
    frame = frameLabel(frame, LED)
    ret, rvec, T = cv2.solvePnP(obj_points, LED.astype('float').reshape(5, 1, 2), marker_udistorder.K, marker_udistorder.D, rvec, T, 1, flags=cv2.SOLVEPNP_UPNP)
    R, jacobian=cv2.Rodrigues(rvec)
    br = tf.TransformBroadcaster()
